...
function sendDataToServer(data, form) {
    const scriptURL = 'https://script.google.com/macros/s/AKfycbzF2D0WwzR_zOFqLt6InFvtYzhay8IlbQTuRz0CcSf2M0wo70yJMHpiVblWvXWK-ZxhwA/exec';//getScriptURL(data['Horario de Congregación']); // Determina la URL adecuada
...
function getScriptURL(horario) {
    const urlBase = 'https://script.google.com/macros/s/AKfycbzF2D0WwzR_zOFqLt6InFvtYzhay8IlbQTuRz0CcSf2M0wo70yJMHpiVblWvXWK-ZxhwA/exec';
...
document.querySelector('#cedula').addEventListener('blur', function () {
    ...
    checkCedulaInServer(cedula)
        .then(exists => {
            if (exists) {
                showCustomAlert('La cédula del beneficiario ya está registrado.\n Por favor, verifica tus datos.', 'error');
...
function checkCedulaInServer(cedula) {
    const scriptURL = 'https://script.google.com/macros/s/AKfycbzF2D0WwzR_zOFqLt6InFvtYzhay8IlbQTuRz0CcSf2M0wo70yJMHpiVblWvXWK-ZxhwA/exec'; // Reemplaza con tu URL de App Script
